...
export const AllGenieObjectInterfaces: GenieInterfacesStore = new GenieInterfacesStore();

export const AllGenieDisplayedInstances: { [key: string]: () => GenieInterfaceSpec } = {};
...
let reactGenieClassModifier: GenieClassModifier = (target: any) => {
    ...
    function Current() {
        ...
        if (ClickPoints.length > 0) {
            ...
            let closestDistance = Math.sqrt(Math.pow(ClickPoints[0].x - closest.rect.x, 2) + Math.pow(ClickPoints[0].y - closest.rect.y, 2))
            for (let i = 1; i < genieInterfaces.length; i++) {
                if (genieInterfaces[i].className !== currentClassName) continue;
                let distance = Math.sqrt(Math.pow(ClickPoints[0].x - genieInterfaces[i].rect.x, 2) + Math.pow(ClickPoints[0].y - genieInterfaces[i].rect.y, 2))
    ...
    target.ClassDescriptor.functions.add(new FuncDescriptor("Current", [], target.ClassDescriptor.className, true))
}
