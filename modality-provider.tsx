...
import {NlInterpreter, ClassDescriptor, ExampleParse, GenieObject} from "reactgenie-dsl";
import {
...
import {displayResult, executeGenieCode, genieCommandSuccess, NavigatorState} from "./shared-store";
...
const DefaultScreen = (children) => ({navigation}) => {
    useEffect(() => {
...
const ObjectInterfaceScreen = (genieInterfaceStoreElement: GenieInterfaceStoreElement) => ({route, navigation}) => {
    let title: string
    ...
    useEffect(
        ...
        , [])
    return (
        <genieInterfaceStoreElement.function {...route.params}/>
    )
}
...
export const ModalityProvider = (props: {
    ...
    const navState: NavigatorState = useSelector(
        (state: any) => {
            return state.navState
        }
    )

    const navStack = useSelector(
        (state: any) => {
            return state.navStack
        }
    )

    const recentVoiceCommand = useSelector(
        (state: any) => {
            return state.command
        }
    )

    const voiceMessage = useSelector(
        (state: any) => {
    ...
    useEffect(() => {
        ...
        if (navState.objectViewClassName !== null) {
            navigationRef.dispatch(StackActions.push(navState.objectViewClassName, navState.objectConstructorParams))
        }
    }, [navStack])
    ...
    const gui = useMemo(() => (<NavigationContainer ref={navigationRef}>
        <Stack.Navigator initialRouteName={"Default GUI"}>
            <Stack.Screen name={"Default GUI"} key={"Default GUI"}
                          component={DefaultScreen(props.children)} options={cardStyle}/>
            {uiList}
        </Stack.Navigator>
    </NavigationContainer>), []);
    ...
    useEffect(() => {
        if (transcript !== "" && (props.autoStart || activeVoice)) {
            ...
            GenieInterpreter.nlParser.parse(transcript).then(
                function (result) {
    ...
    useEffect(() => {
        ...
        if (recentVoiceCommand !== null && recentVoiceCommand !== undefined && recentVoiceCommand.command !== "") {
            GenieInterpreter.nlParser.respond(lastTranscript, recentVoiceCommand.command, recentVoiceCommand.result).then(
                function (result) {
    ...
    return (
        <View style={{flex: 1}}>
            {( (props.autoStart && listening) || processing || activeVoice) &&
                (<div onMouseDown={handleDown} onClick={handleClick} style={{
                    position: "fixed",
                    top: 0,
                    left: 0,
                    ...
                    {props.displayTranscript && <div style={{
                        position: "absolute",
                        bottom: "10%",
                        left: "50%",
                        transform: "translate(-50%, -50%)",
            ...
            <SpeechRecognizer speechStatusCallback={speechStatusCallback} speechResultCallback={speechResultCallback}
                startListening={startListening}/>
            {gui}
            {snackbarVisible &&
            <Snackbar
              message={voiceMessage.message}
              style={{ position: "absolute", start: 16, end: 16, bottom: 100, backgroundColor: voiceMessage.type === "error" ? "#ff0000" : "#000000" }}
            />
            }
        </View>
    );
