...
import { DataClass } from 'reactgenie-dsl';
...
export function InstantiateGenieObject(objectSpec: GenieObjectSpec): any {
    const objectClass = AllGenieObjects[
        ...
        if (ClickPoints.length > 0) {
            ...
            for (let i = 1; i < genieInterfaces.length; i++) {
                ...
                const distance = Math.sqrt(
                    Math.pow(
                        ClickPoints[0].x -
                            (genieInterfaces[i].rect.x +
                                genieInterfaces[i].rect.width / 2),
                        2
                    ) +
                        Math.pow(
                            ClickPoints[0].y -
            ...
            let biggestArea = 0;
            console.log(
                `current called with ${JSON.stringify(genieInterfaces)}`
            );

            for (const genieInterface of genieInterfaces) {
                if (genieInterface.className !== currentClassName) continue;
                const area =
                    genieInterface.rect.width * genieInterface.rect.height;
                console.log();
                if (area > biggestArea) {
...
export function GenieClassInterface(
    ...
    return function (target: any) {
        ...
        const newGenieInterfaceSpec: GenieInterfaceStoreElement = {
            ...
            title: titleFunction as (any) => string,
            function: newTarget,
