...
import {AllGenieObjectInterfaces, InstantiateGenieObject, RetrieveInterfaces} from "./react-decorators";
        ...
        try {
            ...
            if (result.length > 0) {
                ...
                if (lastResult.result.type === "object") {
                    if (lastResult.result.objectType === "string" || lastResult.result.objectType === "int" || lastResult.result.objectType === "boolean") {
                    ...
                    resultStr = JSON.stringify(lastResult.result.value.map((element) => element.value.description()));
                }
            }
        ...
        const genieInterfaces = RetrieveInterfaces()
            ...
            if (!onScreen) {
                ...
                reactGenieState.navState = {
                    objectViewClassName: AllGenieObjectInterfaces.getInterfaces(instantiatedDisplayingObject).viewClassName,
